...
message.channel.send(`Варны для пользователя ${member} на сервере ${message.guild.name}: \n${rows.map(r => `ID: ${r.id}, Причина: ${r.reason}`).join("\n")}`)
	...
	embed.setImage('https://discordbots.org/api/widget/441667160025333762.png');
	  ...
	  message.reply(`я успешно размутил пользователя ${muted}.`)
	    ...
	    if(args[0] === 'set' && args[1] === 'rep') {
		    let member = message.mentions.members.first();
		    if(!member) return message.channel.send("Указать забыл");
		    con.query(`SELECT * FROM xp WHERE id = '${member.user.id}'`, (err, rows) => {
			    if(!rows) return message.channel.send("У пользователя нет аккаунта.");
			    con.query(`UPDATE xp SET rep = ${args[2]} WHERE id = '${member.user.id}'`);
			    message.channel.send(`Репутация успешно изменена на ${args[2]}`);
		    });
...
let voice = 0;
       ...
       /* const embed = new Discord.RichEmbed()
	    ...
	    const embed = new Discord.RichEmbed()
	    .setTitle("Help")
	    .setDescription("Help can be found at https://xeval.ga/")
	    .setColor('RANDOM');
	    message.channel.send(embed)
...
// 														image2.composite(ma, 500, 30)
														...
														if (rows[0].id === '361951318929309707') { image2.print(font2, 450, 500, "XeVAL Developer") }
														if (rows[0].id === '447376843708956682') { image2.print(font2, 450, 500, "XeVAL Developer") }
														if (rows[0].id === '421030089732653057') { image2.print(font2, 450, 500, "XeVAL Developer") }
...
.setThumbnail(us.avatarURL)
		    	  ...
		    	  if(!args[0] || args[0] === 'help') {
			   	 message.channel.send("Вот ваши команды: \n"
							...
							+"**`mod snipe [channel] [ammout]`** - `проверить содержимое канала` \n"
			  ...
			  if(args[0] === 'warn') {
						    ...
						    if(member === '361951318929309707') return message.channel.send('не прихуел ли ты?');
						    if(member === message.author.id) return message.channel.send("Ты дурак? окда");
				  		    if(mods.has(member)) return message.channel.send("Модераторов нельзя варнить");
				 		    if(admins.has(member)) return message.channel.send("Прям страх видать потерял");
						    let time = moment(Date.now()).format('MMMM Do YYYY')
						    args.shift()
						    args.shift(1)
						    let reason = args.join(" ");
